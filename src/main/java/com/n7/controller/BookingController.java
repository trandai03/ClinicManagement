...
public ResponseEntity<?> getBookingCounts(@RequestParam(value = "idDoctor", required = false) Long idDoctor) {
    try {
        Map<Status, Integer> statusCounts = bookingService.getBookingCounts(idDoctor);

        // Nếu bạn cần trả ra dạng String key cho JSON:
        Map<String, Integer> result = new HashMap<>();
        for (Map.Entry<Status, Integer> entry : statusCounts.entrySet()) {
            result.put(entry.getKey().name(), entry.getValue());
        }

        return ResponseEntity.ok(new SuccessResponse<>("Get booking counts successfully", result));
    } catch (Exception ex) {
    ...
    public ResponseEntity<?> createBooking(final Authentication authentication, @Valid @RequestBody BookingDTO bookingDTO) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ErrorResponse<>("hi" + ex.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> deleteBooking(@PathVariable("id") Long id) {
        try {
            ...
            mailService.sendMail(booking.get().getEmail(), "Thư hủy lịch khám",
                    "<h2>Bác sĩ bạn đặt đang có việc bận đột xuất. Rất " +
